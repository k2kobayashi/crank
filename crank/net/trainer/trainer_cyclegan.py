...
class CycleGANTrainer(LSGANTrainer):
    ...
    def update_D(self, batch, loss, phase="train"):
        ...
        # train discriminator
        outputs = self.model["G"].cycle_forward(feats, enc_h, dec_h, enc_h_cv,
                                                dec_h_cv, spkrvec, spkrvec_cv)
    ...
    def calculate_cycleadv_loss(self, batch, outputs, loss):
        ...
        for c in range(self.conf["n_cycles"]):
            for io in ["org", "cv"]:
                lbl = f"{c}cyc_{io}"
                D_inputs = self.get_D_inputs(batch,
                                             outputs[c][io]["decoded"],
                                             label="cv")
                D_outputs = return_sample(D_inputs)
                if self.conf["acgan_flag"]:
                    D_outputs, spkr_cls = torch.split(D_outputs,
                                                      [1, self.n_spkrs],
                                                      dim=2)
                    ...
                    loss["G"] += (self.conf["alpha"]["acgan"] *
                                  loss[f"D_acgan_adv_{lbl}"])
    ...
    def calculate_cycle_discriminator_loss(self, batch, outputs, loss):
        ...
        for c in range(self.conf["n_cycles"]):
            ...
            if self.conf["acgan_flag"]:
                for k in sample.keys():
                    if k in ["real", "org_fake"]:
                        ...
                        h = batch["cv_h"]
                    sample[k], spkr_cls = torch.split(sample[k],
                                                      [1, self.n_spkrs],
                                                      dim=2)
                    ...
                    if not (self.conf["use_real_only_acgan"]
                            and k == "org_fake"):
            ...
            loss["D"] += (self.conf["alpha"]["fake"] * loss[f"D_fake_{lbl}"] +

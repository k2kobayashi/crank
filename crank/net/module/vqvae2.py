...
class VQVAE2(nn.Module):
    ...
    def forward(self,
        ...
        enc, dec, emb_idxs, _, qidxs = self.decode(enc,
                                                   dec_h,
                                                   use_ema=use_ema,
                                                   detach=encoder_detach)
    ...
    def _get_dec_h(self, dec_h, spkrvec):
        if spkrvec is not None:
            ...
            dec_h = spkremb if dec_h is None else torch.cat([dec_h, spkremb],
                                                            axis=-1)
        return dec_h

    def cycle_forward(self, x, org_enc_h, org_dec_h, cv_enc_h, cv_dec_h,
                      org_spkrvec, cv_spkrvec):
        x = x.transpose(1, 2)
        org_dec_h = self._get_dec_h(org_dec_h, org_spkrvec)
        cv_dec_h = self._get_dec_h(cv_dec_h, cv_spkrvec)
        org_enc_h = org_enc_h.transpose(1,
                                        2) if org_enc_h is not None else None
        org_dec_h = org_dec_h.transpose(1,
                                        2) if org_dec_h is not None else None
    ...
    def decode(self, enc, dec_h, use_ema=True, detach=False):
        ...
        for n in reversed(range(self.conf["n_vq_stacks"])):
            # vq
            enc[n] = enc[n] + dec
            emb_idx, emb_idx_qx, qidx = self.quantizers[n](enc[n],
                                                           use_ema=use_ema)
            if detach:
    ...
    def make_dict(self, enc, dec, emb_idxs, qidxs, enc_unmod):
        ...
        encoded_unmod = ([e.transpose(1, 2) for e in enc_unmod]
                         if enc_unmod is not None else None)
        return {
    ...
    def _construct_net(self):
        ...
        for n in range(self.conf["n_vq_stacks"]):
            if n == 0:
                ...
                dec_in_channels = sum([
                    self.conf["emb_dim"][i]
                    for i in range(self.conf["n_vq_stacks"])
                ])
            ...
            self.encoders.append(
                ParallelWaveGANGenerator(
                    ...
                    layers=self.conf["n_layers"][n] *
                    self.conf["n_layers_stacks"][n],
            ...
            self.encoder_receptive_size += self.encoders[
                -1].receptive_field_size
            self.decoder_receptive_size += self.decoders[
                -1].receptive_field_size
...
class Quantizer(nn.Module):
    def __init__(self,
        ...
        self.embedding.weight.data.uniform_(-1.0 / self.emb_size,
                                            1.0 / self.emb_size)
    ...
    def forward(self, x, use_ema=True):
        ...
        embed_idx = torch.matmul(quantized_onehot.float(),
                                 self.embedding.weight)  # (B, T)
        # exponential moving average
        if self.training and self.ema_flag and use_ema:
            self.ema_size = self.decay * self.ema_size + (
                1 - self.decay) * torch.sum(
                    quantized_onehot.view(-1, self.emb_size), 0)
            embed_sum = torch.sum(torch.matmul(x.transpose(1, 2),
                                               quantized_onehot.float()),
                                  dim=0)
            self.ema_w.data = (self.decay * self.ema_w.data +
                               (1 - self.decay) * embed_sum)
            n = torch.sum(self.ema_size)
            self.ema_size = ((self.ema_size + self.eps) /
                             (n + self.emb_size * self.eps) * n)

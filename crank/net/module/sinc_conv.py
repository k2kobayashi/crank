...
#  This module is fully-copied from espnet (k2kobayashi: 2021/03/02)
...
"""
Following codes are implemented by k2kobayashi for k2kobayashi/crank
...
class SincConvPreprocessingLayer(nn.Module):
    def __init__(
        self,
        in_channels=1,
        ...
        self.sinc_conv = SincConv(
            1,
            sincconv_channels,
            sincconv_kernel_size,
            padding=sincconv_channels // 2,
        )
        self.conv1d = Conv1DFeatureExtractionLayers(
            in_channels=sincconv_channels,
            out_channels=out_channels,
            kernel_sizes=kernel_sizes,
        )

    def forward(self, x):
        """Forward for SincNetPreprocessing

        Args:
            x (torch.Tensor): input tensor (B, T, in_channels)

        Returns:
            y (torch.Tensor): output tensor (B, T, out_channels)
        """
        x = self.sinc_conv(x.transpose(1, 2))
        y = self.conv1d(x).transpose(1, 2)
        return y


class Conv1DFeatureExtractionLayers(nn.Module):
    def __init__(self, in_channels=32, out_channels=80, kernel_sizes=[4, 4, 4, 2]):
        super().__init__()

        modules = []
        for n, k in enumerate(kernel_sizes):
            relu = nn.LeakyReLU()
            pool = nn.MaxPool1d(kernel_size=k)
            padding = k // 2
            if n != len(kernel_sizes) - 1:
                conv = nn.Conv1d(
                    in_channels, in_channels, kernel_size=k, padding=padding
                )
            else:
                conv = nn.Conv1d(
                    in_channels, out_channels, kernel_size=k, padding=padding
                )
            modules.append(conv)
            modules.append(relu)
            modules.append(pool)
